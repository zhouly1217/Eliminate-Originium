...
{
    m_topFrame.setWidth(width() * 0.98);
    m_topFrame.setHeight(height() / 150.0);
    m_topFrame.setPosition(width() / 2.0, height() - m_topFrame.height() / 2);

    m_bottomFrame.setWidth(width() * 0.98);
    m_bottomFrame.setHeight(height() / 150.0);
    m_bottomFrame.setPosition(width() / 2.0, m_bottomFrame.height() / 2);

    m_backgroundTexture.load(":/images/background.png");
    m_rightFrame.setWidth(m_topFrame.height() * 0.8);
...
void GameWindow::keyPressEvent(QKeyEvent* event)
{
    ...
    if (m_gameState == GameState::Menu) {
        ...
        if (key == Qt::Key_1) {
            ...
            stateInit();
            qDebug() << "切换到中等难度：4行砖块，小球速度2.0";
        }
        else if (key == Qt::Key_3) {
            m_currentDifficulty = Difficulty::Hard;
            stateInit();
            qDebug() << "切换到困难难度：原有行数，小球速度2.5";
        }
        else {
